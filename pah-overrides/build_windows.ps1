...
$diagExit = Invoke-GuiExeAndWait -FilePath $exe -ArgumentList @("--diagnostics", $diag)
if ($diagExit -ne 0) { throw "Frozen-app diagnostics failed with exit code $diagExit" }
...
$runtimeSelfTest = Join-Path $app "runtime-self-test.json"
$selfTestExit = Invoke-GuiExeAndWait -FilePath $exe -ArgumentList @("--runtime-self-test", $runtimeSelfTest)
if ($selfTestExit -ne 0) { throw "Hidden-child runtime self-test failed with exit code $selfTestExit" }
$selfTestJson = Get-Content $runtimeSelfTest -Raw | ConvertFrom-Json
if (-not $selfTestJson.ok) { throw "Hidden-child runtime self-test reported a visible or unstoppable child process." }
...
if (-not $SkipCorpus) {
    ...
    $batchExit = Invoke-GuiExeAndWait -FilePath $exe -ArgumentList @("--batch", (Join-Path $root "test-corpus\input"), $corpusOutput)
    if ($batchExit -ne 0) { throw "Corpus run aborted with exit code $batchExit" }
